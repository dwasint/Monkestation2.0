		...
		if(3)
			...
			if (!active_record)
				...
				if(diskette && diskette.fields)
					...
					var/can_load = FALSE
					var/mob/living/cardholder = user
					var/obj/item/card/id/C = cardholder.get_idcard(TRUE)
					if(C)
						if(check_access(C))
							can_load = TRUE
					if(can_load)
...
/obj/machinery/computer/cloning/Topic(href, href_list)
	...
	if(href_list["task"])
		...
		active_record = find_record_old("id", href_list["view_rec"], records)
		if(active_record)
			menu = 3
		else
			temp = "Record missing."
		...
		var/datum/data/record/C = find_record_old("id", href_list["clone"], records)
