/mob/living/proc/body_temperature_damage(datum/gas_mixture/environment, seconds_per_tick, times_fired)
	if(bodytemperature > bodytemp_heat_damage_limit && !HAS_TRAIT(src, TRAIT_RESISTHEAT))
		...
		var/effective_temp = bodytemperature * (1 + firemodifier)
		var/burn_damage = HEAT_DAMAGE
		if(bodytemperature > heat_threshold_high)
			burn_damage *= 8
		else if(bodytemperature > heat_threshold_medium)
			burn_damage *= 4
		else if(bodytemperature > heat_threshold_low)
			burn_damage *= 2

		temperature_burns(burn_damage * seconds_per_tick, effective_temp, seconds_per_tick)
		if(bodytemperature > heat_threshold_medium)
	...
	if(bodytemperature < bodytemp_cold_damage_limit && !HAS_TRAIT(src, TRAIT_RESISTCOLD) && (getFireLoss() < maxHealth || stat != DEAD))
		...
		if(bodytemperature > cold_threshold_low)
			cold_damage *= 2
		else if(bodytemperature > cold_threshold_medium)
			cold_damage *= 4
		else if(bodytemperature > cold_threshold_high)
			cold_damage *= 8
...
/mob/living/proc/temperature_homeostasis(seconds_per_tick, times_fired)
	...
	var/natural_change = round((homeostasis_target - bodytemperature) * metabolism_efficiency * temperature_homeostasis_speed, 0.01)
	if(natural_change == 0)
		return
