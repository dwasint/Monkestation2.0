...
/datum/component/uses_mana/airhike/react_to_successful_use(atom/cast_on)
	. = ..()
	drain_mana()

//If there isn't enough mana and the Rclick check passes so it won't mess up any future Normal casts
/datum/component/uses_mana/airhike/can_activate_check_failure(give_feedback, ...)
...
/datum/action/cooldown/spell/airhike/New(Target, original)
	...
	AddComponent(/datum/component/uses_mana/airhike, \
