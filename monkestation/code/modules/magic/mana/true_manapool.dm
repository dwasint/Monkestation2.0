...
/datum/mana_pool
	...
	VAR_PROTECTED/donation_budget_this_tick = BASE_MANA_DONATION_RATE
...
/datum/mana_pool/proc/set_max_mana(new_max, change_amount = FALSE, change_softcap = TRUE)
	...
	if (change_amount)
		var/change_percent = get_percent_to_max()
		amount = new_max * (change_percent / 100)
