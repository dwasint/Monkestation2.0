...
/datum/spellbook_manager/proc/customize_item(datum/spellbook_item/item, datum/spellbook_item_customization_menu/typepath, list/menu_params)
	...
	RegisterSignal(customization_menu, COMSIG_PREQDELETED, TYPE_PROC_REF(/datum/spellbook_manager, cleanup_customization_menu))
...
/datum/spellbook_manager/ui_static_data()
	...
	spellbook_tabs += list(list("name" = "Thaumaturgey", "title" = "Items related to manipulate the paranormal.", "contents" = list_to_data(GLOB.spellbook_thaumaturgy_items)))
	...
	spellbook_tabs += list(list("name" = "Arcane Magic", "title" = "Items related to the manipulation of phenomena", "contents" = list_to_data(GLOB.spellbook_arcane_items)))
