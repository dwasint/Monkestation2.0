...
/datum/chewin_cooking/recipe_tracker
...
/datum/chewin_cooking/recipe_tracker/proc/apply_flat_penalty(var/penalty)
	...
	for (var/datum/chewin_cooking/recipe_pointer/pointer in active_recipe_pointers)
		pointer.tracked_quality -= penalty

//Generate recipe_pointer objects based on the global list
/datum/chewin_cooking/recipe_tracker/proc/generate_pointers()

	#ifdef CHEWIN_DEBUG
	log_debug("Called /datum/chewin_cooking/recipe_tracker/proc/generate_pointers")
	#endif
	var/obj/item/reagent_containers/cooking_container/container = holder_ref.resolve()
	...
	if(GLOB.chewin_recipe_dictionary[container.appliancetype])
		for (var/key in GLOB.chewin_recipe_dictionary[container.appliancetype])
			#ifdef CHEWIN_DEBUG
			log_debug("Loading [container.appliancetype] , [key] into pointer.")
			#endif
			active_recipe_pointers += new /datum/chewin_cooking/recipe_pointer(container.appliancetype, key, src)

//Generate next steps
/datum/chewin_cooking/recipe_tracker/proc/get_step_options()

	#ifdef CHEWIN_DEBUG
	log_debug("Called /datum/chewin_cooking/recipe_tracker/proc/get_step_options")
	#endif
	...
	#ifdef CHEWIN_DEBUG
	log_debug("/datum/chewin_cooking/recipe_tracker/proc/get_step_options returned [options.len] options")
	#endif
	return options


/datum/chewin_cooking/recipe_tracker/proc/populate_step_flags()
	#ifdef CHEWIN_DEBUG
	log_debug("Called /datum/chewin_cooking/recipe_tracker/proc/populate_step_flags")
	#endif
	step_flags = 0
	for (var/datum/chewin_cooking/recipe_pointer/pointer in active_recipe_pointers)
...
/datum/chewin_cooking/recipe_tracker/proc/has_recipes()
	#ifdef CHEWIN_DEBUG
	log_debug("Called /datum/chewin_cooking/recipe_tracker/proc/has_recipes")
	#endif
...
/datum/chewin_cooking/recipe_tracker/proc/process_item_wrap(var/obj/used_object, var/mob/user)
	...
	if(response == CHEWIN_SUCCESS || response == CHEWIN_COMPLETE || response == CHEWIN_PARTIAL_SUCCESS)
		if(!recipe_started)
			recipe_started = TRUE
	return response
...
/datum/chewin_cooking/recipe_tracker/proc/process_item(var/obj/used_object, var/mob/user)
	...
	if(completion_lockout)
		#ifdef CHEWIN_DEBUG
		log_debug("/datum/chewin_cooking/recipe_tracker/proc/process_item held in lockout!")
		#endif
		return CHEWIN_LOCKOUT
	...
	for (var/datum/chewin_cooking/recipe_pointer/pointer in active_recipe_pointers)
		...
		for (var/datum/chewin_cooking/recipe_step/step in option_list)
			...
			#endif
			if(is_valid == CHEWIN_CHECK_VALID)
	...
	if(completed_list.len && (completed_list.len != active_recipe_pointers.len))
		...
		for(var/datum/chewin_cooking/recipe_pointer/pointer in completed_list)
			if(!recipe_string)
	...
	var/datum/chewin_cooking/recipe_pointer/chosen_pointer = null
	...
	var/datum/chewin_cooking/recipe_step/sample_step = valid_steps[1]
	#ifdef CHEWIN_DEBUG
	...
	#endif
	return CHEWIN_PARTIAL_SUCCESS

//Sleep... My precious, monsterous child....
/*
/datum/chewin_cooking/recipe_tracker/proc/attempt_complete_recursive(
		var/obj/used_object,
		var/use_class,
		var/depth = 1,
	...
	for (var/datum/chewin_cooking/recipe_pointer/pointer in ourlist)
		var/option_list = list()
		option_list += pointer.get_possible_steps()
		...
		for (var/datum/chewin_cooking/recipe_step/step in option_list)
			...
			if(depth !=1 && !step.auto_complete_enabled)
				continue
			if(step.check_conditions_met(used_object, src) == CHEWIN_CHECK_VALID)
				has_valid_step = TRUE
			else
				continue
...
/datum/chewin_cooking/recipe_pointer
	var/datum/chewin_cooking/recipe/current_recipe //The recipe being followed here.
	var/datum/chewin_cooking/recipe_step/current_step //The current step in the recipe we are following.
...
/datum/chewin_cooking/recipe_pointer/New(start_type, recipe_id, var/datum/chewin_cooking/recipe_tracker/parent)
	...
	#ifdef CHEWIN_DEBUG
	if(!GLOB.chewin_recipe_dictionary[start_type][recipe_id])
		log_debug("Recipe [start_type]-[recipe_id] not found by tracker!")
	#endif

	current_recipe = GLOB.chewin_recipe_dictionary[start_type][recipe_id]
...
/datum/chewin_cooking/recipe_pointer/proc/get_possible_steps()

	#ifdef CHEWIN_DEBUG
	log_debug("Called /datum/chewin_cooking/recipe_pointer/proc/get_possible_steps")
	if(!current_step)
	...
	for(var/datum/chewin_cooking/recipe_step/step in current_step.optional_step_list)

		if(steps_taken["[step.unique_id]"])
			//Traverse an option chain if one is present.
			if(step.flags & CHEWIN_IS_OPTION_CHAIN)
				var/datum/chewin_cooking/recipe_step/option_chain_step = step.next_step
		...
		if(step.flags & CHEWIN_IS_EXCLUSIVE)
			for (var/id in GLOB.chewin_optional_step_exclusion_dictionary["[step.unique_id]"])
	...
	#ifdef CHEWIN_DEBUG
	log_debug("/datum/chewin_cooking/recipe_pointer/proc/get_possible_steps returned list of length [return_list.len]")
	#endif
	return return_list

//Get the classes of all applicable next-steps for a recipe in a bitmask.
/datum/chewin_cooking/recipe_pointer/proc/get_step_flags()
	#ifdef CHEWIN_DEBUG
	log_debug("Called /datum/chewin_cooking/recipe_pointer/proc/get_step_flags")
	if(!current_step)
		log_debug("Recipe pointer in [current_recipe] has no current_step assigned?")
...
/datum/chewin_cooking/recipe_pointer/proc/traverse(var/id, var/obj/used_obj)
	...
	if(!GLOB.chewin_step_dictionary["[id]"])
		return FALSE
	var/datum/chewin_cooking/recipe_tracker/tracker = parent_ref.resolve()
	var/datum/chewin_cooking/recipe_step/active_step = GLOB.chewin_step_dictionary["[id]"]

	var/is_valid_step =  FALSE
	var/list/possible_steps = get_possible_steps()
	for(var/datum/chewin_cooking/recipe_step/possible_step in possible_steps)
